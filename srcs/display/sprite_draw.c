...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/03/26 14:58:18 by ccommiss          #+#    #+#             */
/*   Updated: 2021/03/26 14:58:20 by ccommiss         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	compute_data(t_data *data, t_spr_geo *sp, t_draw *draw)
{
	...
	if (draw->start_y + data->jump <= 0)
		draw->start_y = -1 - data->jump;
	draw->end_y = sp->height * 0.5 + (data->height) * (data->look);
	if (draw->end_y >= data->height + (fabs)(data->jump))
		draw->end_y = data->height + (fabs)(data->jump);
...
void	draw_text(t_data *data, t_draw *draw, t_spr_geo *sp)
{
	...
	d = ((draw->y - data->jump) * 256 - ((data->height) * 256 * data->look)
			+ (sp->height * 128));
	draw->tex_y = (abs)((d * data->sph) / sp->height) / 256;
	if (data->spw * draw->tex_y + draw->tex_x > 63 * 63
		|| data->spw * draw->tex_y + draw->tex_x < 0)
		return ;
...
**  [call] : in wall_casting (raycasting)
**	[return] : none
*/
...
void	sprite_drawing(t_data *data)
{
	...
	while (data->spr != NULL)
	{
		...
		{
			draw.tex_x = (int)(256 * (draw.start_x - (-sp.width / 2
							+ sp.screen_x)) * data->spw / sp.width) / 256;
